...
class SNS ():
  ...
  def get_sub_fan_sub(self, sub_id, sub_id_dict):   
    ...
    all_fan_id_list = self.get_fans(sub_id)
    print("the number of fans found from the sub id %s : %s "% (sub_id, len(all_fan_id_list)))                
    self.get_all_subs( all_fan_id_list, sub_id_dict)   
  ...
  def write_sub_info(self, sub_id_focus_pair_list, sub_id_dict, this_level):
  	fp = open("url_"+first_sub_id+"_"+str(this_level)+"m.htm",'w')        
  	for  _pair in sub_id_focus_pair_list:
        	 sub_id = _pair[0]
        	 focus_num = _pair[1]
  	...
  	sub_id_list.sort()
  	fp = open("id_"+first_sub_id+"_"+str(this_level)+"m.txt",'w')        
